...
public class ScreenShotListenManager {
	...
	public void startListen() {
		...
		this.internalObserver = new MediaContentObserver(Media.INTERNAL_CONTENT_URI, this.mUiHandler);
		this.externalObserver = new MediaContentObserver(Media.EXTERNAL_CONTENT_URI, this.mUiHandler);
		this.mContext.getContentResolver().registerContentObserver(Media.INTERNAL_CONTENT_URI, false, this.internalObserver);
		this.mContext.getContentResolver().registerContentObserver(Media.EXTERNAL_CONTENT_URI, false, this.externalObserver);
	}
	...
	public void stopListen() {
		...
		if(this.internalObserver != null) {
			try {
				this.mContext.getContentResolver().unregisterContentObserver(this.internalObserver);
		...
		if(this.externalObserver != null) {
			try {
				this.mContext.getContentResolver().unregisterContentObserver(this.externalObserver);
	...
	private void handleMediaContentChange(Uri contentUri) {
		Cursor cursor = null;
		try {
			cursor = this.mContext.getContentResolver().query(contentUri, VERSION.SDK_INT < 16 ? MEDIA_PROJECTIONS:MEDIA_PROJECTIONS_API_16, 
